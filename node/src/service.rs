...
// Runtime
use cloudwalk_network_testnet_runtime::{opaque::Block, RuntimeApi};
...
impl sc_executor::NativeExecutionDispatch for ExecutorDispatch {
	...
	fn dispatch(method: &str, data: &[u8]) -> Option<Vec<u8>> {
		cloudwalk_network_testnet_runtime::api::dispatch(method, data)
	}

	fn native_version() -> sc_executor::NativeVersion {
		cloudwalk_network_testnet_runtime::native_version()
	}
}
	...
	if role.is_authority() {
		...
		impl sp_inherents::InherentDataProvider for MockTimestampInherentDataProvider {
			fn provide_inherent_data(
				...
				TIMESTAMP.with(|x| {
					*x.borrow_mut() += cloudwalk_network_testnet_runtime::SLOT_DURATION;
